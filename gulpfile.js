...
gulp.task('deploy', ['jade', 'less', 'js', 'image'], function(cb) {
  exec('git add -f build; git commit -m "tmp"; git push origin `git subtree split --prefix build master`:gh-pages --force;git reset HEAD^;git reset build', function(err, stdout, stderr) {
